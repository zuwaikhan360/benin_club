...
export default async function handler(
  ...
  try {
    ...
    if (payment.data.status === "successful") {
      ...
      const message =
        "We hope this message finds you well. We are delighted to inform you that your recent form fee payment has been successfully processed. Thank you for your prompt action in completing this important step to maintain your membership with Benin";
      sendEmail(user.email, "Form Fee Payment", message);

      const message1 = `New form fee payment by ${user.firstName} ${user.surName}`;
      for (let i = 0; i < usersAdminEmails.length; i++) {
        sendEmail(usersAdminEmails[i], "Form Fee Payment", message1);
      }
